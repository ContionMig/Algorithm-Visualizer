...
int ImGui::PlotEx(ImGuiPlotType plot_type, const char* label, float (*values_getter)(void* data, int idx), void* data, int values_count, int values_offset, const char* overlay_text, float scale_min, float scale_max, ImVec2 frame_size, int HighLightedIndex, int HighLightedIndex2)
{
    ImGuiContext& g = *GImGui;
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
    ...
    if (values_count >= values_count_min)
    {
        ...
        for (int n = 0; n < res_w; n++)
        {
            ...
            else if (plot_type == ImGuiPlotType_Histogram)
            {
                ...
                if (HighLightedIndex > -1 && HighLightedIndex2 > -1)
                {
                    if (v1_idx == HighLightedIndex || v1_idx == HighLightedIndex2)
...
void ImGui::PlotHistogram(const char* label, const float* values, int values_count, int values_offset, const char* overlay_text, float scale_min, float scale_max, ImVec2 graph_size, int stride, int Hightlight, int HighLightedIndex2)
{
    ImGuiPlotArrayGetterData data(values, stride);
    PlotEx(ImGuiPlotType_Histogram, label, &Plot_ArrayGetter, (void*)&data, values_count, values_offset, overlay_text, scale_min, scale_max, graph_size, Hightlight, HighLightedIndex2);
}
